...
export const handler = async (
  event: { cod_citacao_dado: string; ind_conferido_contencioso?: boolean },
  context: Context
) => {
  console.info('event');
  ...
  try {
    return await atualizaCitacaoIntegrada({
      cod_citacao_dado: event.cod_citacao_dado.toString(),
      ind_conferido_contencioso: event.ind_conferido_contencioso ?? true,
    });
  } catch (error: any) {
    console.error('ERRO:');
